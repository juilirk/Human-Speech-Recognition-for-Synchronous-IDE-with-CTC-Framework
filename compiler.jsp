 ...
 %> <!-- Edit Solve Program -->
								...
								<form action="CompilerServlet" method="post">
									...
									<table border="1" cellpadding="5" cellspacing="3">
										...
										<tr align="center">
											<td>Assignment: <%=asignment%><input type="hidden" name="as_id" value="<%=ass_id%>"/><input type="hidden"
 	...
 	if (request.getParameter("nas_id") != null) {
 		...
 		ResultSet rs1 = stm1
 				.executeQuery("select * from assignment where assignment_id='"+ nas_id + "'");
 ...
 %>
								...
								<form action="CompilerServlet" method="post">
									<table border="1" cellpadding="5" cellspacing="3">
										...
										<tr align="center">
											<td>Assignment:-<%=rs1.getString("assignment")%> <input type="hidden" name="assignment" value="<%=rs1.getString("assignment")%>"> <input
							...
							<%
								if (request.getParameter("as_id") != null) {
									...
									PreparedStatement ps_1 = conn
											.prepareStatement("SELECT * FROM `solve_assignment` where stud_as_id="
													+ aas_id);
									...
									if (rs_1.next()) {
										...
										r_op = rs_1.getString("ass_status");
